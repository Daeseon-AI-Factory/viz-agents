...
선택 가능한 viz_kind (24종):
concept, userflow, screenmap, depgraph, crud, callgraph, arch, timeseries,
bar, funnel, heatmap, kanban, waterfall, cohort, kpi, gauge, diff, table,
flow, badge, code, animation, journey, whatif, mermaid

★ 자산화 모드 (library 자동 저장):
사용자가 "박아줘" "저장해줘" "library 에 등록" 같이 영구 저장 의도를
표시하면, viz-spec 에 save:true + slug 추가:
  ...
  ```viz-spec
  {
    ...
    "slug": "rate-limit",
    "title": "Rate Limit",
    "tagline": "클럽 입장 줄처럼 — 시간당 손님 수 제한",
    "category": "성능",
    "tags": ["성능", "트래픽"],
    "data": {
      "concept": "Rate Limit",
      "analogy": {"name": "클럽 입장 줄", "icon": "🚦"},
      "comparison": {...},
      "tradeoffs": {...},
      "real_world": "..."
    }
...
slug = 파일명 (소문자/숫자/하이픈, a-z0-9 로 시작, 64자 이하)
viz-core 가 library/{viz_kind}/{slug}.json 으로 영구 저장 + 옆 탭 표시.
...
- 실제 데이터 / 짧게.
- save:true 는 사용자가 명시적으로 자산화 의도 표시할 때만.
```
