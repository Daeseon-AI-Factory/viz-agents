...
app = FastAPI(title="VIZ V26 Live HUD")
...
async def _analyze_security(event_data: dict) -> None:
    ...
    prompt = (
        f"보안 분석. 파일: {fp}\n코드:\n{new_code[:1500]}\n\n"
        f"SQL injection / XSS / 비밀 노출 / 위험 함수 / 권한 문제 등 보안 이슈 있나?\n"
        f"JSON만: {{\"severity\": \"none|low|medium|high|critical\", \"issues\": [\"한국어 50자 이내씩\"]}}"
    )
    try:
        ...
        if parsed.get("severity", "none") == "none":
            return  # 이슈 없으면 broadcast 안 함
        record = {
            "ts": _now_iso(),
            "data": {
                "hook_event_name": "_security",
