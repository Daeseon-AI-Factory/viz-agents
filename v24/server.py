...
- code는 100줄 이내
- ★ V20 우선: 사용자 피드백 학습 가정 — 같은 컨텍스트에 일관된 viz_kind 선택.
- JSON 외 다른 텍스트, 마크다운, 코드펜스 절대 금지
"""

app = FastAPI(title="VIZ V24 Live HUD")
...
async def github_pr(request: Request) -> dict[str, Any]:
    ...
    import re as _re_g
    m = _re_g.match(r"https?://github\.com/([^/]+)/([^/]+)/pull/(\d+)", pr_url)
    if not m:
        return {"ok": False, "error": "GitHub PR URL 형식이 아님"}
    ...
    try:
        ...
        if resp.status_code != 200:
            return {"ok": False, "error": f"LLM {resp.status_code}"}
        ...
        try:
            ...
            return {"ok": False, "error": "LLM JSON 파싱 실패"}
        record = {
            "ts": _now_iso(),
            "data": {
                "hook_event_name": "_summary",
                "session_id": f"_github_pr_{owner}_{repo}_{num}",
                "reason": "github_pr",
                "event_count": 0,
                "llm_used": True,
                "pr_url": pr_url,
...
async def feedback(request: Request) -> dict[str, Any]:
    ...
    if record["vote"] not in ("up", "down"):
        return {"ok": False, "error": "vote는 up/down"}
    try:
