...
viz_kind 가이드 (작업 종류에 따라 다르게):
  ...
  - journey : ★ V7 핵심 — 사용자 여정 단계
             viz_data: {"persona": "신규 가입자", "stages": [{"name":"방문", "icon":"👤", "drop": 0}, {"name":"가입", "icon":"📝", "drop": 10}, ...]}
             각 stage: name, icon, drop(이탈%). 라우터/페이지 작업이면 우선.
...
- code는 100줄 이내
- ★ V7 우선: 라우터/페이지/엔드포인트 작업이면 viz_kind "flow" + 사용자 여정 단계 표현 (방문 → 액션 → 결과).
- JSON 외 다른 텍스트, 마크다운, 코드펜스 절대 금지
"""

app = FastAPI(title="VIZ V7 Live HUD")
