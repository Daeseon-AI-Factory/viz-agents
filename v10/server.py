...
viz_kind 가이드 (작업 종류에 따라 다르게):
  ...
  - animation : ★ V10 핵심 — 노드 사이 흐르는 애니메이션 (본인 비전 직격)
             viz_data: {"nodes": [{"id":"node1", "label":"API"}, ...], "flow": ["node1","node2","node3"], "duration_ms": 2500}
             예: API→Service→DB 요청 흐름, 마이크로서비스 호출 시퀀스
             "흐름" 성격이면 flow 보다 animation 을 선호.
...
app = FastAPI(title="VIZ V10 Live HUD")
