...
viz_kind 가이드 (작업 종류에 따라 다르게):
  ...
  - arch   : ★ V8 핵심 — 아키텍처 레이어 박스
             viz_data: {"layers": [{"name": "Frontend", "components": ["Next.js", "CDN"]}, {"name": "API", "components": ["FastAPI"]}, ...], "active_layer": "API"}
...
- code는 100줄 이내
- ★ V8 우선: 인프라/배포/docker/k8s 파일 작업이면 viz_kind "flow" 로 레이어 명시 (Frontend / API / Service / Data / Infra).
- JSON 외 다른 텍스트, 마크다운, 코드펜스 절대 금지
"""

app = FastAPI(title="VIZ V8 Live HUD")
