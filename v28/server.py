...
app = FastAPI(title="VIZ V28 Live HUD")
...
async def _daily_retro_loop() -> None:
    ...
    while True:
        ...
        prompt = (
            ...
            f"JSON: {{\"summary\": \"3-5문장 회고\", \"highlights\": [\"잘된 것 1-3개\"], \"next\": \"내일 할 일\"}}"
        )
        try:
            async with httpx.AsyncClient(timeout=25.0) as client:
                resp = await client.post(
            ...
            record = {
                "ts": _now_iso(),
                "data": {"hook_event_name": "_retro", "session_id": "_daily_retro", **parsed},
            }
            async with _lock:
                _history.append(record)
            await _broadcast(record)
...
async def _on_startup() -> None:
    ...
    asyncio.create_task(_daily_retro_loop())  # ★ V28
