...
- code는 100줄 이내
- ★ V6 우선: 여러 파일/모듈을 만지는 작업이면 viz_kind "flow" 로 모듈 흐름 강조. summary 에 만진 모듈 명시.
- JSON 외 다른 텍스트, 마크다운, 코드펜스 절대 금지
"""

app = FastAPI(title="VIZ V6 Live HUD")
...
# ── V6 핵심: topology endpoint ──
@app.get("/topology")
async def topology() -> dict[str, Any]:
    """Claude가 만진 파일들 + 그 사이 import 관계 그래프"""
    import re as _re_t

    touched_files: set[str] = set()
    async with _lock:
        for ev in _history[-200:]:
            d = ev.get("data", {})
    ...
    for fp in touched_files:
        try:
            ...
            imports = _re_t.findall(r'^\s*(?:from\s+(\S+)\s+)?import\s+([^\n]+)', content, _re_t.MULTILINE)
            src = Path(fp).stem
