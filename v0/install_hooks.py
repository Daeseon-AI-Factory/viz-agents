"""Install VIZ V0 hooks into ~/.claude/settings.json.

Idempotent — checks for a tagged comment in the command so re-running won't duplicate.
Creates a timestamped backup of settings.json before modifying.
"""
...
def main() -> int:
    ...
    print(f"\n신규 {installed}개 / 스킵 {skipped}개 / 총 {len(EVENTS)}개 이벤트")
    print(f"파일: {SETTINGS_PATH}")
    print("\nClaude Code 세션을 재시작하면 hooks 적용됩니다.")
    return 0
