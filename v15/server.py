...
- code는 100줄 이내
- ★ V15 우선: 사용자가 자연어로 시각 요청한 거면 그 요청 의도 정확히 매핑.
- JSON 외 다른 텍스트, 마크다운, 코드펜스 절대 금지
"""

app = FastAPI(title="VIZ V15 Live HUD")
...
async def viz_request(request: Request) -> dict[str, Any]:
    ...
    if not user_request:
        return {"ok": False, "error": "request 필드 없음"}
    ...
    try:
        async with httpx.AsyncClient(timeout=20.0) as client:
            resp = await client.post(
                ...
                json={
                    "model": MODEL, "max_tokens": 800, "system": SUMMARY_SYSTEM,
                    "messages": [{"role": "user", "content": user_msg}],
                },
            )
        if resp.status_code != 200:
        ...
        try:
            ...
            return {"ok": False, "error": "LLM 응답 JSON 파싱 실패"}
        record = {
            "ts": _now_iso(),
            "data": {
                "hook_event_name": "_summary",
                "session_id": "_user_request",
                "reason": "on_demand",
                "event_count": 0,
                "llm_used": True,
