...
- code는 100줄 이내
- ★ V5 우선: 코드 diff 발견 시 단순 패치 X — "보안 수정/리팩터링/기능 추가/버그 픽스" 의미 라벨링. summary 첫 부분에 [태그] 형식.
- JSON 외 다른 텍스트, 마크다운, 코드펜스 절대 금지
"""

app = FastAPI(title="VIZ V5 Live HUD")
...
# ── V5 핵심: Python symbol diff (regex 기반, AST 대신 가벼움) ──
import re as _re

def _extract_py_symbols(code: str) -> dict:
    """Python 코드에서 함수/클래스/import 추출 (regex, partial 코드도 OK)"""
        ...
        if name == "UserPromptSubmit":
            ...
            if tn in ("Read", "Edit", "Write", "NotebookEdit"):
                ...
                if tn in ("Edit", "Write", "NotebookEdit"):
                    ...
                    # V5: Python 파일이면 symbol diff 추출
                    fp = item.get("file", "")
                    if fp.endswith(".py"):
                        old_code = inp.get("old_string") or ""
                        new_code = inp.get("new_string") or inp.get("content") or ""
                        diff = _diff_py_symbols(old_code, new_code)
                        if any(diff.values()):
